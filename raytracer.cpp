...
int main(){
  ...
  Vector green(60,167,28);
    ...
    plane p(Vector(128,128,100),Vector(0,-2,1),red,.5,1.5,1,50);
  ...
  for (int y = 0; y<256; y++){
    for(int x=0;x<256;x++){
        ...
        if (p.intersect(e, d, t) && t > 0) {
            ...
            double la = p.ka * (intensity);
            la = (la * y/192);
            if (p.color.x * la > 255) {
                img(x, y, 0) = 255;
            } else {
                img(x, y, 0) = p.color.x * la;
            }
            if (p.color.y * la > 255) {
                img(x, y, 1) = 255;
            } else {
                img(x, y, 1) = p.color.y * la;
            }
            if (p.color.z * la > 255) {
                img(x, y, 2) = 255;
            } else {
                img(x, y, 2) = p.color.z * la;
            }
        }
      ...
      for(int i = 0;i<sizeof(objs)/sizeof(objs[0]);i++) {
          ...
          if (objs[i].sphere.intersect(e, d, t) && t > 0) {
              ...
              double ls = objs[i].ks * (intensity) *
                          pow(std::max(0.0, dot(normal.normalize(), r.normalize())), objs[i].pow);
